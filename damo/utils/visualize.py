...
def vis(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    ...
    for i in range(len(boxes)):
        ...
        if class_names[cls_id] not in general_config.DESIRED_OBJECT['parking_monitoring']:
            continue
        ...
        color = (_COLORS[1] * 255).astype(np.uint8).tolist()
        text = '{}:{:.1f}%'.format(class_names[cls_id], score * 100)
        txt_color = (0, 0, 0) if np.mean(_COLORS[1]) > 0.5 else (255, 255,
                                                                      255)
